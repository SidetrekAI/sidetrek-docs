...
### Running Meltano Ingestion in Dagster
...
<BashCode code="select count(*) from orders;" />
...
### Running Meltano Ingestion Manually

We just ran the ingestion job inside Dagster, but you can also run it manually by executing the following command:
