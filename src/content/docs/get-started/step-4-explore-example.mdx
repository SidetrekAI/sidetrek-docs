...
We've included a simple dataset with four tables: `attendance` (205800 rows), `employees` (200 rows), `performance` (3200 rows), and `salary` (7932 rows). This emulates the typical HR records.
...
|-----------------|-----------|------------------------------------------|--------------------------|
| ID              | String    | Unique identifier for the record         | Primary Key, Unique      |
| Employee ID     | Integer   | ID of the employee                       | Foreign Key to Employees |
| Date            | Date      | Date of the attendance record            |                          |
| Status          | String    | Attendance status (e.g., Present/Absent) |                          |
| Clock In Time   | Time      | Time employee clocked in                 |                          |
| Clock Out Time  | Time      | Time employee clocked out                |                          |
</div>
</div>
...
| Column Name       | Data Type | Description                           | Constraints           |
|-------------------|-----------|---------------------------------------|-----------------------|
| Employee ID       | Integer   | Unique ID for each employee           | Primary Key, Unique   |
| Name              | String    | Full name of the employee             |                       |
| Department        | String    | Department the employee belongs to    |                       |
| Role              | String    | Job title or role of the employee     |                       |
| Date of Joining   | Date      | Date the employee joined the company  |                       |
| Resignation Date  | Date      | Date the employee left the company    | Nullable              |
| Age               | Integer   | Age of the employee                   |                       |
| Supervisor ID     | Integer   | ID of the employee's supervisor       | Foreign Key to Employees |
</div>
</div>
</div>
...
| Column Name           | Data Type | Description                                               | Constraints              |
|-----------------------|-----------|-----------------------------------------------------------|--------------------------|
| ID                    | String    | Unique identifier for the performance record              | Primary Key, Unique      |
| Employee ID           | Integer   | ID of the employee being evaluated                        | Foreign Key to Employees |
| Evaluation Date       | Date      | Date of the performance evaluation                        |                          |
| Review Period         | String    | Review period (e.g., "Q1 2022", "Q2 2022")               |                          |
| Score                 | Integer   | Performance score (e.g., 1-5 scale)                       |                          |
| KPI_1                 | Integer   | Score for KPI 1                                           |                          |
| KPI_2                 | Integer   | Score for KPI 2                                           |                          |
| KPI_3                 | Integer   | Score for KPI 3                                           |                          |
| Goal Achievement      | Integer   | Goal achievement percentage                               |                          |
| Promotion Eligible    | Boolean   | Eligibility for promotion (True if Score > 4)             |                          |
| Training Recommended  | Boolean   | Recommendation for training (True if Score < 2)           |                          |
</div>
</div>
...
| Column Name         | Data Type | Description                                           | Constraints              |
|---------------------|-----------|-------------------------------------------------------|--------------------------|
| ID                  | String    | Unique identifier for the salary record               | Primary Key, Unique      |
| Employee ID         | Integer   | ID of the employee                                    | Foreign Key to Employees |
| Month               | Date      | Month of the salary record (YYYY-MM)                  |                          |
| Base Salary         | Float     | Base salary amount                                    |                          |
| House Rent          | Float     | House rent allowance                                  |                          |
| Transportation      | Float     | Transportation allowance                              |                          |
| Communication       | Float     | Communication allowance                               |                          |
| Wellness            | Float     | Wellness allowance                                   |                          |
| Tax Deduction       | Float     | Tax deduction amount                                  |                          |
| Net Salary          | Float     | Final net salary after deductions                     |                          |
...
<BashCode code="select count(*) from attendance;" client:load />
