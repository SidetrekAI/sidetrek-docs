...
The example code already includes deduping logic in the `staging` stage, but let's see how this works.

TODO
Show the deduping logic in the `staging` stage.
Show the number of rows in the raw table to show that there's 100k
Show the number of rows in the staging table to show that there's 100k
Run the ingestion again in dagster
Show the number of rows in the table to show that it's deduped via Trino shell.
Show the number of rows in the staging table to show that there's 100k
Explain row_num method for deduping
Run DBT assets in dagster (reload def first)
Show the number of rows in the staging table to show that there's still only 100k
