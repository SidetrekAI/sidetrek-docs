...
<BashCode code="poetry run dbt --version" client:load />

### Running All Models

You can directly run DBT models using CLI:
<BashCode code="poetry run dbt run" client:load />

### Running A Specific Model

To run a specific model or a list of models, you can use the `--select` flag:

<BashCode code="poetry run dbt run --select <model-name>" client:load />
For example:
<BashCode code="poetry run dbt run --select marts_iceberg__general" client:load />
...
<BashCode code="poetry run dbt run --select +stg_iceberg__>" client:load />

### Excluding A Model or A List of Models
You can also exclude a specific model or a list of models using the `--exclude` flag:

<BashCode code="poetry run dbt run --exclude <model-name>" client:load />
For example:
<BashCode code="poetry run dbt run --exclude int_iceberg__denormalized_orders" client:load />
